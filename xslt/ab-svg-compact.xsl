...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:result-document method="xhtml" indent="yes" href="../site/img/svg/edition-svg.svg">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="300" height="8000" viewBox="0 0 300 8000">
                                                    ...
                                                    <g class="ab{count(preceding::ab) + 1}-{$CE}-variance{position()}-string-count">
                                                        <xsl:choose>
                                                            <xsl:when test="position() = (count(parent::ab/app) - 1)">
                                                                <text id="ab{count(preceding::ab) + 2}" x="26" y="{((count(preceding::app) * 20) + 160) + ((count(preceding::ab) - 1) * 80)}" text-anchor="middle">
