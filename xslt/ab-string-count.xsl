...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:result-document method="xhtml" indent="yes" href="../site/img/svg//edition-svg-string-count-bar-graph.svg">
            <div id="string-count" class="ab-graph" style="-webkit-user-select: none">
                            ...
                            <xsl:for-each select="$currentEdition">
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="current() = 'W'">
                                        <g class="w-markers" id="ab{$CAB/count(preceding::ab) + 1}-string">
                                            <xsl:choose>
                                                <xsl:when test="(sum($CAB//rdg[contains(@wit, $CE)]/text() ! string-length())) = 0">
                                                    <text x="20" y="{(position() * 30) + 10}"><xsl:value-of select="(sum($CAB//rdg[contains(@wit, $CE)]/text() ! string-length()))"/></text>
                                                </xsl:when>
