...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:template match="/">
        <svg xmlns="http://www.w3.org/2000/svg" width="500" height="6000">
            ...
            <line x1="0" y1="0" x2="0" y2="-{$timeLength * $stretcher}" stroke="ForestGreen" stroke-width="3"/>
               ...
               <xsl:for-each select="0 to $timeLength">
                   <xsl:if test="current() mod 20 = 0"><text x="0" y="-{current() * $stretcher}" text-align="center" stroke="black"><xsl:value-of select="$lowest + current()"/></text></xsl:if>
                   
               </xsl:for-each>
                <xsl:for-each select="descendant::entry">
                    <xsl:choose>
                        <xsl:when test="@when"><circle r="5" cx="0" cy="-{(@when - $lowest) * $stretcher}" stroke="black" fill=""/><text></text></xsl:when>
                        <xsl:when test="@from"><ellipse rx="8" ry="{(@to - @from) * $stretcher}" cx="0" cy="-{(@from - $lowest) * $stretcher}" stroke="black" fill="pink" fill-opacity="0.5"/></xsl:when>
                    </xsl:choose>
                </xsl:for-each>
        </g>
        </svg>
